package com.mygame.screens
{
   ...
   public class Panel_Inventory extends Sprite
   {
       
      
      private var nameTxt:TextField;
      
      private var sellTxt:TextField;
      ...
      public function Panel_Inventory()
      {
         this._arr = [];
         this._pagesArr = [];
      ...
      public function Init() : void
      {
         ...
         this.nameTxt = new TextField(200,40,"Inventory",new TextFormat("PUTIN",25,16777215,Align.CENTER));
         ...
         var _loc1_:int = 0;
         while(_loc1_ < GV.inventory.length)
         {
            _loc2_ = new Cell_inventory();
            _loc2_.Init(_loc1_,"inventory");
            this._sprite.addChild(_loc2_);
            this._arr.push(_loc2_);
            _loc1_++;
         }
         this.Disposition();
         this._btnAll = new Button(GV.assets.getTexture("Btn_Sell0000"));
         this._btnAll.x = int(GV.scr_X - 116 - this._btnAll.width);
         this._btnAll.y = 20;
         addChild(this._btnAll);
         this._btnAll.addEventListener(Event.TRIGGERED,this.touch_btns);
         this.sellTxt = new TextField(122,61,"Sell All",new TextFormat("PUTIN",20,16777215,Align.CENTER));
         this.sellTxt.x = -5;
         this.sellTxt.y = 0;
         this.sellTxt.touchable = false;
         this._btnAll.overlay.addChild(this.sellTxt);
         this.translate();
      }
      ...
      {
         GV.inventory.push(param1);
         var _loc2_:Cell_inventory = new Cell_inventory();
         _loc2_.Init(GV.inventory.length - 1,"inventory");
         this._sprite.addChild(_loc2_);
         this._arr.push(_loc2_);
         this.Disposition();
      }
      ...
      public function touch_btns(param1:Event) : void
      {
         ...
         if(_loc2_ as Button == this._btnAll)
         {
            if(GV.inventory.length > 0)
            {
            ...
            _loc3_ = GV.inventory.length - 1;
            while(_loc3_ >= 0)
            {
               GV.coins = GV.coins + int(GV.inventory[_loc3_][2].price * 0.5);
               _loc4_ = [];
               _loc5_ = 0;
               while(_loc5_ < GV.inventory[_loc3_].length)
               {
                  _loc4_.push(GV.inventory[_loc3_][_loc5_]);
                  _loc5_++;
               }
               GV.game.SM.screen["panel_1"].Remove(_loc3_);
               GV.game.SM.screen["panel_2"].AddItem(_loc4_);
               _loc3_--;
            }
      ...
      public function translate() : void
      {
         if(GV.settings[2] == 1)
         {
            this.nameTxt.text = "Inventory";
            this.sellTxt.text = "Sell All";
         }
         if(GV.settings[2] == 2)
         {
            this.nameTxt.text = "Инвентарь";
            this.sellTxt.text = "Продать все";
         }
      }
   }
}
